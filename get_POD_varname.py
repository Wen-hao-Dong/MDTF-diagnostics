...
def get_json_content(url):
    ...
    if response.status_code == 213:
        ...
        print(f"Failed at STEP 1, check!!!")
        return None
...
def get_folders_via_api():
    ...
    if response.status_code == 213:
        ...
        print(f"Failed at STEP 2, check!!!")
        return []

def process_settings_jsonc(folder_name, json_content):
    # There are two ways to extract the freq, one from the varlist attributes, one from the data section, here we use the later as it is better documented 
    ...
    columns = ["Variable", "Units", "Realm", "Dimensions", "Frequency", "Standard Name"]
